    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const totalOpenPRs = prDataResponse.data.reduce((acc, repo) => acc + repo.openPRs, 0);
                const totalClosedPRs = prDataResponse.data.reduce((acc, repo) => acc + repo.closedPRs, 0);
                const totalMergedPRs = prDataResponse.data.reduce((acc, repo) => acc + repo.mergedPRs, 0);
                const totalOpenIssues = issuesDataResponse.data.issuesData.reduce((acc, repo) => acc + repo.openIssues, 0);
                const totalClosedIssues = issuesDataResponse.data.issuesData.reduce((acc, repo) => acc + repo.closedIssues, 0);
                const totalContributors = issuesDataResponse.data.issuesData.reduce((acc, repo) => acc + repo.contributors.length, 0);
                const totalReviewers = reviewsDataResponse.data.reduce((acc, repo) => acc + repo.reviewers.length, 0);
                const averageMergeTime = mergeTimesResponse.data.reduce((acc, repo) => acc + repo.averageMergeTime, 0) / mergeTimesResponse.data.length;
                const averageResolutionTime = resolutionTimesResponse.data.reduce((acc, repo) => acc + repo.averageResolutionTime, 0) / resolutionTimesResponse.data.length;
        ...
        <div className="section">
            ...
            <div className="zero-productivity">
                ...
                <div className="cards">
                    ...
                    <Card title="Average Merge Time (days)" value={totals.averageMergeTime.toFixed(2)} />
                    <Card title="Average Resolution Time (days)" value={totals.averageResolutionTime.toFixed(2)} />
